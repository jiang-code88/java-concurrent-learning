 ...
 * - 对于那些不会发生变化的共享变量，建议你用 final 关键字来修饰，
 *   这样既能避免并发问题，也能很明了地表明你的设计意图，你已经考虑过这些共享变量的并发安全问题了。
 *
 * - 当代码中出现 if 语句时，就应该立即意识到可能存在的竞态条件，会导致线程不安全情况的发生。
 */
...
/**
 * 使用 AtomicLong 线程安全的原子类来
 * 表示成员变量 upper 和 lower，表示库存上限和下限，
 * 这样这两个成员变量的 set 方法就不需要同步了。
 */
class SafeWM{ // 库存管理
    // 仓库库存上限
...
/**
 * 如果增加「库存的上限要小于库存下限」约束条件
 * 这里其实就存在并发问题，问题在于存在竞态条件（当代码中出现 if 语句时，就应该立即意识到可能存在的竞态条件）
 * 最后结果会不符合「库存的上限要小于库存下限」约束条件。
 */
class SafeWM_A {
    // 仓库库存上限
    private final AtomicLong upper = new AtomicLong(0);
