 ...
 *    CAS 指令是硬件 CPU 支持的 CPU 指令，由硬件保证指令执行的原子性。
 *
 * 原子类概述：
 *  1）原子化的基本数据类型
 *   - AtomicBoolean
 ...
 *   那么不管 A 怎么从 B 又变化为 A，版本号都是递增，都可以通过版本号记录到其变化。
 *
 ...
 *   - LongAccumulator
 *   - LongAdder
 *   这四个类仅仅用来执行累加操作，相比原子化的基本数据类型，累加速度更快，
...
public class _18_Atomic {
    ...
    //  - 原子化的更新对象需要关注 ABA 问题，
    //    因为两个 A 虽然相等（对象地址相同），但是第二个 A 的对象内部属性可能已经发生变化了。
